...
const Canvas = () => {
	...
	const [storedImages, setStoredImages] = useState([]); //stores unused and used images
		...
		if (file && file.type.startsWith('image/')) {
			...
			reader.onload = (evt) => {
				...
				img.onload = () => {
					...
					setStoredImages(findDuplicateImage(storedImages, newImage));
				};
			};
	...
	//allows images to be uploaded and stores them in setStoredImages
	const handleImageUpload = (e) => {
		...
		reader.addEventListener(
			...
			() => {
				...
				//only adds images when the src is not the same between rest of array
				setStoredImages(findDuplicateImage(storedImages, newImage));
			},
			false,
		);
			...
			<div style={{ position: 'absolute', top: 10, left: 10, zIndex: 10 }}>
				...
				<div>
					...
					<input
						...
						className="m-2 imageUpload"
					/>
					<ImageBar storedImages={storedImages} />
				</div>
			</div>
